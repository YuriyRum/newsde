...
import { NewsItem, ViewLayout, ThemeMode, FontSize, AppLanguage } from './types.ts';
...
export default function App() {
  ...
  const [language, setLanguage] = useState<AppLanguage>(() => {
    try {
      const saved = localStorage.getItem('deutschland_news_language');
      return (saved as AppLanguage) || 'de';
    } catch {
      return 'de';
    }
  });
  ...
  useEffect(() => {
    localStorage.setItem('deutschland_news_language', language);
  }, [language]);
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-3 sm:pt-5 w-full flex-1">
        ...
        <div className="flex items-center justify-between text-xs text-stone-500 dark:text-stone-400 mb-4 pb-2 border-b border-stone-200/80 dark:border-stone-800/80">
          <div className="flex items-center gap-2">
            <span className="inline-flex items-center gap-1.5 font-semibold text-stone-700 dark:text-stone-300">
              ...
              {filteredItems.length} {t('articlesCount', language)}
            </span>
            {searchQuery && (
              <span className="italic">{t('forSearch', language)} „{searchQuery}“</span>
            )}
            {selectedCategory !== 'all' && (
              ...
              >
                <span>{selectedCategory === 'breaking' ? t('breakingCategory', language) : getLocalizedCategoryName(selectedCategory, language)}</span>
          ...
          <div className="flex items-center gap-3">
            {lastUpdated && (
              <span className="hidden sm:inline-flex items-center gap-1">
                ...
                {t('updatedAt', language)} {lastUpdated.toLocaleTimeString(language === 'ru' ? 'ru-RU' : 'de-DE', { hour: '2-digit', minute: '2-digit' })} {t('oClock', language)}
              </span>
            )}
            <div className="flex items-center gap-1 text-emerald-700 dark:text-emerald-400 font-medium">
              <ShieldCheck className="w-3.5 h-3.5" />
              <span className="hidden md:inline">{t('adFreeView', language)}</span>
            </div>
          </div>
        </div>
        ...
        {error && items.length === 0 && (
          <div className="py-16 text-center space-y-4 max-w-md mx-auto">
            ...
            >
              <RotateCw className="w-4 h-4" /> {t('retry', language)}
            </button>
          </div>
        )}
        ...
        {!loading && filteredItems.length === 0 && !error && (
          <div className="py-20 text-center space-y-4 max-w-md mx-auto">
            ...
            <p className="text-xs text-stone-500 dark:text-stone-400">
              {searchQuery
                ? `${t('noSearchResults', language)} „${searchQuery}“.`
                : t('noEntriesForSelection', language)}
            </p>
